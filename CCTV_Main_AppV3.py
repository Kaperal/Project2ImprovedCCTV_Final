...
class CCTVApp:
    ...
    def run_audio_detection_thread(self, microphone_id):
        def audio_callback(indata, frames, time, status):
            ...
            # Process audio data
            detected_event = self.detect_event(indata)  # Example method for event detection
            if detected_event in ['gun', 'scream']:
                self.alert_flag = True
            else:
                self.alert_flag = False

        # Use sounddevice to start audio detection
        stream = sd.InputStream(device=microphone_id, callback=audio_callback)
        stream.start()
