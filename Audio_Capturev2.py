...
class AudioRecorderApp:
    ...
    def record_audio(self):
        ...
        try:
            device_info = sd.query_devices(self.selected_audio_device)
            max_channels = device_info['max_input_channels']
            channels = min(2, max_channels)  # Use 2 channels if supported, otherwise use the maximum available

            messagebox.showinfo("Recording", "Recording will start now. Please speak!")
            audio_data = sd.rec(int(duration * fs), samplerate=fs, channels=channels, dtype='int16', device=self.selected_audio_device)
            sd.wait()  # Wait for recording to finish
            self.save_audio_file(file_path, audio_data, fs, channels)
    ...
    def save_audio_file(self, file_path, audio_data, sample_rate, channels):
